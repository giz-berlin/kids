...
pub trait Target {
    ...
    /// Only returns [identifiers](types::ResourceIdentifier) because the ground truth for the
    ...
    async fn delete_group(group: types::ResourceIdentifier) -> Result<(), error::KidsError>;
    /// Delete the user with the given identifier from the target system.
    async fn delete_user(group: types::ResourceIdentifier) -> Result<(), error::KidsError>;

    /// Update group attributes and authorization details in the target system to match those of the given
    /// [source group](source::interface::Group).
    ...
    /// Update user attributes and authorization details in the target system
    ///
