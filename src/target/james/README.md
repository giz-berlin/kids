...
Synchronizes source groups and users to James users, lists and teams.
...
If the `james-team` is set, we create a James team with `source-uuid` as team name under the domain specified in `james_team_domain`. Additionally, we create an alias email address for the team email address for each email in the `james-team` attribute. Therefore, if multiple `james-team` attributes are set on a single source group, all map to the same team in James.

If the `james-list` is set, we do not need to create something because list are created by adding a first user. But we create an alias email address for the list email address (source-uuid@DOMAIN, where DOMAIN has the value of `james_list_domain`) for each email in the `james-list` attribute. Therefore, if multiple `james-list` attributes are set on a single source group, all map to the same list in James.
...
Source group memberships are handled via the user sync.
...
The team mailbox will only be deleted, if the source group itself gets deleted.
...
A source user has also a groups field. With this field the user's group membership is managed. We update the memberships according to that field in James. If the first user is added to a list, the list is automatically created. List have the form of source-uuid@DOMAIN, where DOMAIN has the value of `james_list_domain`. If the last user of a list is removed, the list is automatically deleted. 
...
The source may contain email addresses from a variety of domains. However, James should only process email addresses belonging to domains that are permitted. To ensure that user or group alias email addresses are created only when their email address domain is supported by James.
