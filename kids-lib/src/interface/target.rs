...
pub trait Target: Sized {
    ...
    /// group attributes is the [Source](crate::interface::source::Source).
    ...
    /// Only returns [identifiers](types::SharedResourceIdentifier) because the ground truth for the
    /// user attributes is the [Source](crate::interface::source::Source).
    ...
    async fn create_or_update_group(&mut self, group: std::sync::Arc<dyn crate::interface::source::Group + Send + Sync>) -> Result<(), error::KidsError>;
    ...
    async fn create_or_update_user(&mut self, user: std::sync::Arc<dyn crate::interface::source::User + Send + Sync>) -> Result<(), error::KidsError>;
}
