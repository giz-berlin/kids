...
pub trait JamesApi {
    async fn get_users(&mut self) -> Result<Vec<dto::User>, KidsError>;
    async fn create_user(&mut self, user_email: &str) -> Result<(), KidsError>;
    async fn delete_user(&mut self, user_email: &str) -> Result<(), KidsError>;
    async fn get_lists(&mut self) -> Result<Vec<String>, KidsError>;
    async fn get_list_members(&mut self, list_email: &str) -> Result<Vec<String>, KidsError>;
    async fn add_member_to_list(&mut self, list_email: &str, user_email: &str) -> Result<(), KidsError>;
    async fn remove_member_from_list(&mut self, list_email: &str, user_email: &str) -> Result<(), KidsError>;
    async fn get_aliases_of(&mut self, email: &str) -> Result<Vec<dto::Alias>, KidsError>;
    async fn add_alias(&mut self, email: &str, alias_email: &str) -> Result<(), KidsError>;
    async fn remove_alias(&mut self, email: &str, alias_email: &str) -> Result<(), KidsError>;
    async fn get_teams(&mut self) -> Result<Vec<dto::Team>, KidsError>;
    async fn create_team(&mut self, team_id: &str) -> Result<(), KidsError>;
    async fn delete_team(&mut self, team_id: &str) -> Result<(), KidsError>;
    async fn get_team_members(&mut self, team_id: &str) -> Result<Vec<dto::Member>, KidsError>;
    async fn add_member_to_team(&mut self, team_id: &str, user_email: &str) -> Result<(), KidsError>;
    async fn remove_member_from_team(&mut self, team_id: &str, user_email: &str) -> Result<(), KidsError>;
    async fn get_user_teams(&mut self, user_email: &str) -> Result<Vec<dto::Team>, KidsError>;

    async fn get_domains(&mut self) -> Result<Vec<String>, KidsError>;
}
...
impl JamesClient {
    pub async fn new(config: JamesApiConfig) -> Result<Self, KidsError> {
...
impl JamesApi for JamesClient {
    ...
    async fn get_teams(&mut self) -> Result<Vec<dto::Team>, KidsError> {
        let teams: Vec<dto::Team> = self
            .send_api_get_request(format!("/domains/{}/team-mailboxes", urlencoding::encode(&self.config.james_team_domain)))
            .await?;
        Ok(teams)
    }
