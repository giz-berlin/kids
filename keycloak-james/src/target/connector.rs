...
/// A connector to James providing the [Target](kids_lib::interface::target::Target) interface.
...
impl kids_lib::interface::target::Target for Connector {
    ...
    async fn all_groups(&mut self) -> Result<collections::HashSet<types::SharedResourceIdentifier>, KidsError> {
        Ok(self.get_cached_group_id_mapping().await?.keys().cloned().collect())
    }

    async fn all_users(&mut self) -> Result<collections::HashSet<types::SharedResourceIdentifier>, KidsError> {
        Ok(self.get_cached_user_ids().await?.iter().cloned().collect())
    }

    async fn delete_group(&mut self, group_id: &types::SharedResourceIdentifier) -> Result<(), KidsError> {
    ...
    async fn delete_user(&mut self, user_id: &types::SharedResourceIdentifier) -> Result<(), KidsError> {
        ...
        match self.james_api.delete_user(&uuid_user_email).await {
            Ok(_) => tracing::info!(user_id = user_id, "Delete user"),
    ...
    async fn create_or_update_group(&mut self, source_group: std::sync::Arc<dyn kids_lib::interface::source::Group + Sync + Send>) -> Result<(), KidsError> {
    ...
    async fn create_or_update_user(&mut self, source_user: std::sync::Arc<dyn kids_lib::interface::source::User + Sync + Send>) -> Result<(), KidsError> {
        ...
        let desired_source_groups = source_user.groups(true).await.map_err(|e| {
...
impl Connector {
    ...
    fn create_uuid_user_email(&self, source_user_id: &str) -> Result<String, KidsError> {
    ...
    fn create_uuid_list_email(&self, source_group_id: &str) -> Result<String, KidsError> {
    ...
    fn create_uuid_team_email(&self, source_group_id: &str) -> Result<String, KidsError> {
    ...
    async fn domain_contained_in_cached_james_domains(&mut self, domain: &String) -> Result<bool, KidsError> {
    ...
    async fn get_cached_user_ids(&mut self) -> Result<&mut collections::HashSet<types::SharedResourceIdentifier>, KidsError> {
    ...
    async fn get_cached_group_id_mapping(&mut self) -> Result<&mut collections::HashMap<types::SharedResourceIdentifier, dto::Group>, KidsError> {
    ...
    async fn update_alias(&mut self, uuid_email: &str, desired_aliases: &[String]) -> Result<(), KidsError> {
    ...
    async fn delete_all_aliases_and_members_from_team(&mut self, group_id: &str, team_uuid_email: &str, team_aliases: &[String]) -> Result<(), KidsError> {
        self.update_alias(team_uuid_email, team_aliases).await?;
    ...
    async fn delete_all_aliases_and_members_from_list(&mut self, list_uuid_email: &str, list_aliases: &[String]) -> Result<(), KidsError> {
        self.update_alias(list_uuid_email, list_aliases).await?;
        self.delete_all_members_from_list(list_uuid_email).await?;
        Ok(())
    }

    async fn delete_all_members_from_list(&mut self, list_uuid_email: &str) -> Result<(), KidsError> {
