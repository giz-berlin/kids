...
OTHER_CHILD_GROUP_ROOM_ID=$(synadm --config-file "$SYNADM_CONFIG_FILE" room list | jq -r ".rooms.[] | select(.canonical_alias == \"#testgroup-the_other_subgroup:$PODMAN_SERVICE_HOSTNAME:$SYNAPSE_TLS_PORT\").room_id")
...
check_room "$OTHER_CHILD_GROUP_ROOM_ID" "Matrix child room" "#testgroup-the_other_subgroup:$PODMAN_SERVICE_HOSTNAME:$SYNAPSE_TLS_PORT" "$ADMIN_USER_NAME" 1 "$SYNADM_CONFIG_FILE"
