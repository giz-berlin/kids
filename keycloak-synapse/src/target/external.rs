...
impl SynapseClient {
    ...
    async fn send_mas_admin_request_list_get<T: serde::de::DeserializeOwned>(&self, path: ApiPath) -> Result<dto::mas::ListResponse<T>, KidsError> {
    ...
    async fn client_api_get<T: serde::de::DeserializeOwned>(&self, path: ApiPath) -> Result<T, KidsError> {
    ...
    async fn admin_api_get<T: serde::de::DeserializeOwned>(&self, api_version: &str, path: ApiPath) -> Result<T, KidsError> {
...
impl SynapseApi for SynapseClient {
    ...
    async fn get_users(&self) -> Result<Vec<crate::target::types::User>, KidsError> {
        let mas_users: Vec<dto::mas::UserResponse> = self.send_mas_admin_request_list_get(ApiPath::from_segments(["users"])).await?.data;
    ...
    async fn get_user_three_pids(&self, matrix_user_id: &str) -> Result<Vec<dto::synapse::ThreePID>, KidsError> {
        let response: dto::synapse::User = self.admin_api_get("v2", ApiPath::from_segments(["users", matrix_user_id])).await?;
    ...
    async fn get_user_emails(&self, mas_user_id: &dto::mas::internal::user::Id) -> Result<Vec<String>, KidsError> {
        let emails: Vec<dto::mas::UserEmailResponse> = self
            .send_mas_admin_request_list_get(ApiPath::from_segments_and_query(["user-emails"], [("filter[user]", mas_user_id.as_str())]))
            .await?
            .data;
    ...
    async fn set_user_emails(&self, mas_user_id: &dto::mas::internal::user::Id, emails: &[String]) -> Result<(), KidsError> {
        ...
        for user_email in user_emails_to_remove {
            self.send_mas_admin_request::<(), dto::IgnoredResponse>(
                http::Method::DELETE,
                ApiPath::from_segments(["user-emails", user_email.id.as_str()]),
                None,
            )
            .await?;
        }
        ...
        for email in emails_to_add {
            self.send_mas_admin_request::<_, dto::IgnoredResponse>(
                http::Method::POST,
                ApiPath::from_segments(["user-emails"]),
                Some(serde_json::json!({
                    "user_id": mas_user_id,
                    "email": email,
                })),
    ...
    async fn get_user_display_name(&self, matrix_user_id: &str) -> Result<Option<String>, KidsError> {
        let response: dto::matrix::UserDisplayNameResponse = self.client_api_get(ApiPath::from_segments(["profile", matrix_user_id, "displayname"])).await?;
    ...
    async fn create_user(&self, matrix_user_id: &str, source_user_id: &str) -> Result<crate::target::types::User, KidsError> {
        ...
        self.send_mas_admin_request::<_, dto::IgnoredResponse>(
            http::Method::POST,
            ApiPath::from_segments(["upstream-oauth-links"]),
            Some(serde_json::json!({
    ...
    async fn get_source_user_id_for_mas_user_id(
        ...
        let response: Vec<dto::mas::UpstreamOauthLinkResponse> = self
            .send_mas_admin_request_list_get(ApiPath::from_segments_and_query(
    ...
    async fn join_user_to_room(&self, matrix_group_id: &str, matrix_user_id: &str) -> Result<(), KidsError> {
        let _: dto::IgnoredResponse = self
            .send_admin_api_request(
                "v1",
                http::Method::POST,
    ...
    async fn kick_user_from_room(&self, matrix_group_id: &str, matrix_user_id: &str) -> Result<(), KidsError> {
        let _: dto::IgnoredResponse = self
            .send_client_api_request(
                http::Method::POST,
