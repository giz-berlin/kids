...
pub struct KentixApiConfig {
    ...
    /// The [Kentix Usernames](super::dto::Username) of users we will ignore.
    ///
    ...
    pub kentix_ignored_usernames: Vec<super::dto::Username>,
    ...
    ///
    /// **Note**: No other certificates will be accepted if this option is provided.
    ...
    pub fn new(config: KentixApiConfig) -> Self {
        ...
        let http_client = {
            ...
            if let Some(kentix_root_certificate_pem_path) = config.kentix_root_certificate_pem_path.as_ref() {
                tracing::warn!("Accepting only root certificate stored at {}.", kentix_root_certificate_pem_path.display());
                let kentix_root_certificate_pem = std::fs::read(kentix_root_certificate_pem_path)
                    .unwrap_or_else(|err| panic!("Cannot read PEM certificate at {}: {err}", kentix_root_certificate_pem_path.display()));
...
impl KentixApi for KentixClient {
    ...
    async fn get_users(&self) -> Result<Vec<super::dto::UserWithId>, kids_lib::error::KidsError> {
        ...
        let user_ids = self
            ...
            .await?
            .into_iter()
            .filter(|user: &SparseUser| !self.config.kentix_ignored_usernames.contains(&user.username))
            .map(|user| user.id);
    ...
    async fn create_user(&self, user: super::dto::User) -> Result<super::dto::UserWithId, kids_lib::error::KidsError> {
        if self.config.kentix_ignored_usernames.contains(&user.username) {
    ...
    async fn update_user(&self, user: super::dto::UserWithId) -> Result<super::dto::UserWithId, kids_lib::error::KidsError> {
        if self.config.kentix_ignored_usernames.contains(&user.user.username) {
