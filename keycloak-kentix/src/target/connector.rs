    ...
    async fn create_or_update_user(
        ...
        let (rfid_uid, rfid_data) = {
            ...
            match (rfid_uid.map(Vec::as_slice), rfid_data.map(Vec::as_slice), existing_kentix_user.as_ref()) {
                (Some([rfid_uid]), rfid_data_slice_opt @ (Some([_]) | Some([]) | None), _) => (
                    ...
                    rfid_data_slice_opt.and_then(|rfid_data_slice| rfid_data_slice.first().map(|rfid_data| super::dto::UserRfidData(rfid_data.clone()))),
                ),
                (Some(_), Some(_), _) => {
                    const ERROR_CONTEXT: &str = "Cannot add or create user";
                ...
                // Accounts without a key, e.g. management accounts, are not managed via KIDS (see `kentix_ignored_usernames` of `KentixApiConfig` in this context).
                (_, _, Some(_)) => {
                    tracing::debug!(
                        source_user_id = source_user.id(),
                        "User no longer has the required RFID UID attached. Deleting them."
                    );
                    return self.delete_user(source_user.id()).await;
                }
                (_, _, None) => {
                    tracing::trace!(source_user_id = source_user.id(), "Skipping user without required RFID UID attached.");
                    return Ok(());
                }
            }
        };
...
mod test {
    ...
    const RFID_UID_ATTRIBUTE_NAME: &str = "kentixUid";
    const RFID_DATA_ATTRIBUTE_NAME: &str = "kentixData";
        ...
        Connector {
            config: KentixConfig {
                kentix_api: crate::target::external::KentixApiConfig {
                    kentix_ignored_usernames: vec!["admin".into()],
    ...
    mod manage_users {
        ...
        mod create {
            ...
            #[rstest::rstest]
            #[tokio::test]
            async fn create_user_noop_without_rfid_uid(mut connector: Connector) {
                // given
        ...
        mod update {
            ...
            #[rstest::rstest]
            #[tokio::test]
            async fn deletes_user_when_rfid_uid_no_longer_present(mut connector: Connector) {
                // given
                let available_levelprofiles = [
